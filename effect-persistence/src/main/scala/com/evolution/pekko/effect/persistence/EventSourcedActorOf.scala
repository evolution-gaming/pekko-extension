...
import com.evolution.pekkoeffect.persistence.{EventStore, Events, SeqNr, SnapshotStore}
...
object EventSourcedActorOf {
  ...
  // format: off
  /** Describes lifecycle of entity in regard to event sourcing & PersistentActor Lifecycle phases:
    ...
    * Termination phase described via aggregation of [[cats.effect.Resource]] release callbacks. Please refer to phase types for
    * more details.
    *
    * @tparam S snapshot
    * @tparam E event
  ...
  /**
   * Factory method aimed to create [[org.apache.pekko.actor.Actor]] capable of handling commands,
   * saving snapshots and producing events. The actor uses Event Sourcing pattern to persist
   * events/snapshots and recover state from them later.
   *
   * Actor's lifecycle is described by type [[Lifecycle]] and consists of multiple phases, such as
   * recovering, receiving messages and terminating. Recovery happens on actor's startup and is
   * about constructing latest actor state from snapshot and followed events. On receiving phase
  ...
  private[evolution] def receiveOf[F[_]: Async: LogOf, S, E](
    eventSourcedOf: EventSourcedOf[F, Lifecycle[F, S, E, Any]],
    persistence: EventSourcedPersistence[F, S, E],
  ...
  private[evolution] implicit final class SnapshotOps[S](val snapshot: SnapshotStore.Offer[S]) extends AnyVal {
  ...
  private[evolution] implicit final class SnapshotStoreOps[F[_], A](val store: SnapshotStore[F, A]) extends AnyVal {
  ...
  private[evolution] implicit final class EventStoreOps[F[_], E](val store: EventStore[F, E]) extends AnyVal {
