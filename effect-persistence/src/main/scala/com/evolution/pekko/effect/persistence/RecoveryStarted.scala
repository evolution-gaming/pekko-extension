...
object RecoveryStarted {
  ...
  private[RecoveryStarted] final class Apply[S](private val b: Boolean = true) extends AnyVal {
    ...
    ): RecoveryStarted[F, S, E, A] = { (seqNr, snapshotOffer) =>
      f(seqNr, snapshotOffer)
    }
  }

  def const[F[_], S, E, A](
    recovering: Resource[F, Recovering[F, S, E, A]],
  ): RecoveryStarted[F, S, E, A] = { (_, _) =>
    recovering
  }
  ...
  implicit class RecoveryStartedOps[F[_], S, E, A](
    ...
    def convert[S1, E1, A1](
      ...
      F: Monad[F],
    ): RecoveryStarted[F, S1, E1, A1] = { (seqNr, snapshotOffer) =>
      ...
      } yield recovering.convert(sf, ef, e1f, af)
    }

    def map[A1](f: A => A1): RecoveryStarted[F, S, E, A1] = { (seqNr, snapshotOffer) =>
      self(seqNr, snapshotOffer).map(_.map(f))
    }

    def mapM[A1](f: A => Resource[F, A1]): RecoveryStarted[F, S, E, A1] = { (seqNr, snapshotOffer) =>
  ...
  implicit class RecoveryReceiveEnvelopeOps[F[_], S, E, C](
    ...
    def widen[S1 >: S, C1 >: C, E1 >: E](
      ...
      F: Monad[F],
    ): RecoveryStarted[F, S1, E1, Receive[F, Envelope[C1], Boolean]] = { (seqNr, snapshotOffer) =>
      val snapshotOffer1 = snapshotOffer.traverse { snapshotOffer =>
