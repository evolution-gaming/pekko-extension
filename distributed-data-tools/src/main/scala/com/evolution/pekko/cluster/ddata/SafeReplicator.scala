 ...
 * The data is accessed with an actor providing a key-value store like API. The keys are unique
 * identifiers with type information of the data values. The values are Conflict Free Replicated
 * Data Types (CRDTs).
 *
 * All data entries are spread to all nodes, or nodes with a certain role, in the cluster via direct
 * replication and gossip based dissemination. You have fine grained control of the consistency
 * level for reads and writes.
 */
trait SafeReplicator[F[_], A <: ReplicatedData] {
   ...
   *
   * @param consistency
   *   [[https://pekko.apache.org/docs/pekko/current/typed/distributed-data.html#consistency]]
   */
  def get(
    implicit
   ...
   *
   * To modify and replicate a data value The current data value for the key of the Update is passed
   * as parameter to the modify function of the Update. The function is supposed to return the new
   * value of the data, which will then be replicated according to the given consistency level.
   *
   * @param modify
   *   The modify function is called by the Replicator actor and must therefore be a pure function
   *   that only uses the data parameter and stable fields from enclosing scope. It must for example
   *   not access the sender (sender()) reference of an enclosing actor.
   * @param consistency
   *   [[https://pekko.apache.org/docs/pekko/current/typed/distributed-data.html#consistency]]
   */
  def update(
   ...
   *
   * A deleted key cannot be reused again, but it is still recommended to delete unused data entries
   * because that reduces the replication overhead when new nodes join the cluster. Subsequent
   * delete, update and get calls will fail with Deleted or AlreadyDeleted. Subscribers will stop
   *
   * @param consistency
   *   [[https://pekko.apache.org/docs/pekko/current/typed/distributed-data.html#consistency]]
   */
   ...
   *
   * @param onStop
   *   called when the subscriber is stopped or data is deleted
   * @param onChanged
   *   called when the data is updated.
   * @return
   *   Unsubscribe function, you should call to unregister
   */
  def subscribe(
    onStop: F[Unit],
    onChanged: A => F[Unit],
...
object SafeReplicator {
    ...
    new SafeReplicator[F, A] {
      ...
      def subscribe(
        ...
        def actor() = new Actor with ActorLogging {
          ...
          val handleError: Throwable => F[Unit] = (error: Throwable) => {
  ...
  object Metrics {
    ...
    def const[F[_]](unit: F[Unit]): Metrics[F] = new Metrics[F] {
      ...
      def latency(name: String, latency: FiniteDuration): F[Unit] = unit

      def size(size: Long): F[Unit] = unit
    }
    ...
    object DataMetrics {
      ...
      def empty[F[_]: Applicative, A <: ReplicatedData]: DataMetrics[F, A] = new DataMetrics[F, A] {
        def apply(metrics: Metrics[F], a: A): F[Unit] = ().pure[F]
      }

      private def size[F[_], A <: ReplicatedData](size: A => Int): DataMetrics[F, A] = new DataMetrics[F, A] {
        def apply(metrics: Metrics[F], a: A): F[Unit] = metrics.size(size(a).toLong)
      }
    }
  }
