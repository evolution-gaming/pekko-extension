...
trait ShardingStrategy[F[_]] {
   ...
   *
   * @param requester
   *   actor reference to the [[org.apache.pekko.cluster.sharding.ShardRegion]] that requested the
   *   location of the shard, can be returned if preference should be given to the node where the
   *   shard was first accessed
   * @param shard
   ...
   *   returned set
   * @return
   *   an effect `F` of the shards to be migrated, may be empty to skip rebalance in this round
   */
...
object ShardingStrategy {
  ...
  object AllocationStrategyProxy {
    ...
    def apply[F[_]: FlatMap: ToFuture](
      ...
      new ShardAllocationStrategy {
        ...
        def rebalance(current: Allocation, inProgress: Set[Shard]) = {
          val allocation =
            if (inProgress.isEmpty) current else current.map { case (k, v) => (k, v filterNot inProgress) }
          val shards = for {
  ...
  object Logging {
    ...
    def apply[F[_]: Monad](
      ...
      def iterToStr(iter: Iterable[?]): String = {
      ...
      new ShardingStrategy[F] {
        ...
        def allocate(requester: Region, shard: Shard, current: Allocation) = {
          for {
            ...
            msg = () =>
              s"allocate $shard to ${ region.fold("none") { regionToStr } }, " +
        ...
        def rebalance(current: Allocation, inProgress: Set[Shard]) = {
          for {
            ...
            _ <- if (shards.nonEmpty) {
              val msg = () =>
                s"rebalance ${ shards mkString "," }, " +
                  s"${ if (inProgress.nonEmpty) s"inProgress(${ inProgress.size }): ${ iterToStr(inProgress) }, "
                    else "" }" +
                  s"current: ${ allocationToStr(current) }"
  ...
  /**
   * Adds a track of unallocated shards. If we configured sharding in such a way that it
   * automatically re-create shards and entities inside of it after a rebalance, we know that all
   * shards returned from rebalance call would be allocated in the nearest future. Term unallocated
   * shard in this context means such shard that was stopped on a previous node, but not yet
   * allocated on a new one.
   */
