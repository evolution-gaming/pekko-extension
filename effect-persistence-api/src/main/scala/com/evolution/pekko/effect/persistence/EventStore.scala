...
object EventStore {
  ...
  trait Read[F[_], A] {
     ...
     * @return
     *   lazy event stream of type [[com.evolutiongaming.sstream.Stream]]
     */
  ...
  trait Write[F[_], A] {
    ...
    /**
     * Persist events in batches. Outer [[cats.data.NonEmptyList]] of [[Events#values]] can be used
     * for batching while inner [[cats.data.NonEmptyList]] must be persisted atomically.
     *
     * @param events
  ...
  /**
   * Highest persisted [[SeqNr]] representation. Used as marker of sequence persisted number in case
   * if events not available (were deleted).
   *
   * @param seqNr
