 ...
 *
 * In other words, as soon as the error happens, the effects will not be executed anymore, but the
 * previous error will be returned instead.
 */
trait CloseOnError[F[_]] {
   ...
   *
   * This method is fine to be called several times, but all of the wrapped values will use a single
   * latch, so if one of them fails, all others will return the same error.
   */
