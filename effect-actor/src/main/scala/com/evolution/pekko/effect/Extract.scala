 ...
 *
 * The class is useful in the context of untyped Pekko to represent the conversion from `Any` coming
 * to an actor to a useful type.
 *
...
object Extract {
  ...
  def either[F[_]: Monad, L, R](
    implicit
    l: Extract[F, L],
    r: Extract[F, R],
  ): Extract[F, Either[L, R]] = {
  ...
  implicit class ExtractOps[F[_], A](val self: Extract[F, A]) extends AnyVal {
    ...
    def orElse(
      ...
      F: Monad[F],
    ): Extract[F, A] = a => self(a) orElse extract(a)
  }
}
