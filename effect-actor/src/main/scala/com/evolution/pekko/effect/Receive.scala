...
trait Receive[F[_], -A, B] {
  ...
  /**
   * Called strictly sequentially, next message will be processed only after we've done with the
   * previous one This basically preserves the semantic of Actors
   */
  def apply(msg: A): F[B]
...
object Receive {
  ...
  implicit class ReceiveOps[F[_], A, B](val self: Receive[F, A, B]) extends AnyVal {
    ...
    def map[B1](
      ...
      F: Functor[F],
    ): Receive[F, A, B1] = new Receive[F, A, B1] {
    ...
    def contramapM[A1](
      ...
      F: FlatMap[F],
    ): Receive[F, A1, B] = new Receive[F, A1, B] {
  ...
  implicit class ReceiveCallOps[F[_], A, B, C](val self: Receive[F, Call[F, A, B], C]) extends AnyVal {
    ...
    def toReceiveEnvelope(
      from: Option[ActorRef],
    )(implicit
      F: Sync[F],
    ): Receive[F, Envelope[A], C] =
